...
export const displayProducts = (items, wrapper, rows_per_page, page) => {
  ...
  const start = rows_per_page * page; // 0 , 50 , 100 , 150 ....etc
  const end = start + rows_per_page; // 50 , 100 ,150 ...etc
  const paginatedElements = items.slice(start, end); // (0,50) , (50,100) , (100,150)...etc
  ...
  for (let i = 0; i < pageCount; i++) {
    ...
    button.addEventListener("click", () =>
      displayProducts(items, wrapper, rows_per_page, i)
    );
