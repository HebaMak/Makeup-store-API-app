...
export const cateBrandHandler = (data) => {
  ...
  const categories = new Set(
    allCategories.filter((cat) => cat != null && cat != "")
  );
  ...
  const brands = new Set(
    allBrands.filter((brand) => brand != null && brand != "")
  );
  ...
  const proTypes = new Set(
    allProducts.filter((type) => type != null && type != "")
  );
  ...
  allItems.forEach((item) => {
    ...
    item.addEventListener("click", () => {
      ...
      const filteredData = data.filter((product) =>
        [product.category, product.brand, product.product_type].includes(
          itemText
        )
      );
